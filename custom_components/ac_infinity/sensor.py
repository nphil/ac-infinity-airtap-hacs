...
from .supply_air import ConditionedAirAverage
...
async def async_setup_entry(
    ...
    entities: list[ACInfinitySensor] = [
        ...
        ConditionedAirSensor(data.coordinator, data.device, data.settings,
                             "cooling", "Cold air temperature"),
        ConditionedAirSensor(data.coordinator, data.device, data.settings,
                             "heating", "Warm air temperature"),
    ]
...
class ConditionedAirSensor(ACInfinitySensor, RestoreSensor):
    """Average temperature of the cold (or warm) air this vent delivered.

    One value per HVAC cycle: the steady part of the last cooling (heating)
    cycle, measured at 1 Hz on the fan's own thermistor; see supply_air.py
    for what counts. Long-term statistics of it are the daily average, so
    the vents can be compared side by side for duct losses.

    Needs the thermostat option (CONF_THERMOSTAT): without it nothing says
    when conditioned air is flowing, and the sensor stays unknown.
    """
    ...
    def __init__(
        ...
        self._average = ConditionedAirAverage(action)

    @property
    def available(self) -> bool:
        # A finished cycle's average stays true while the fan is offline.
        return True
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if (last_state := await self.async_get_last_state()) is not None:
            self._attr_extra_state_attributes = {
                ...
                for key in ("cycle_ended", "measured_minutes")
                if key in last_state.attributes
            }
        thermostat = self._settings.thermostat
        if thermostat:
            self.async_on_remove(
    ...
    @callback
    def _async_action(self, state: State | None) -> None:
        action = (
            None
            if state is None or state.state in ("unavailable", "unknown")
            else state.attributes.get("hvac_action")
        )
        result = self._average.on_action(action, monotonic())
        if result is None:
            return
        self._attr_native_value = round(result, 1)
        self._attr_extra_state_attributes = {
            "cycle_ended": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "measured_minutes": round(self._average.measured_seconds / 60, 1),
        }
        self.async_write_ha_state()

    @callback
    def _update_attrs(self) -> None:
        self._average.on_temperature(self._device.temperature, monotonic())
